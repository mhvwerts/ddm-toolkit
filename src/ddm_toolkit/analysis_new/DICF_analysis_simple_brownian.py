...
def DICFanalysis_simple_brownian(DICFA, D_guess, refine_guess = True,
                                verbose = False):
    """Analyze a DICF using the simple Brownian model.
    
    The DICF is also know as the '(image) structure function'
    ...
    Parameters
    ----------
    DICFA : DifferentialImageCorrelationFunction
        The calculated DICF/image structure function.
    D_guess : float
    ...
    result = DICFanalysis_simple_brownian_result()
    result.D_guess = D_guess
    result.tau = DICFA.tau
    result.q = DICFA.q
    ...
    # radial averager
    DICFAtauq = np.zeros((DICFA.Ntau, len(DICFA.u)))
    for i in range(DICFA.Ntau):
        DICFAtauq[i,:] = DICFA.radavg(i)
    # remove "zero-frequency garbage" at center
    DICFAtauq[:,0] = 0.0

    # store in result dictionary
    # for external plotting
    result.radISF_q = DICFA.q
    result.radISF_tau = DICFA.tau
    result.radISF_tauq = DICFAtauq[:,:]
    ...
    tau_max = DICFA.tau[-1]
    deltau = DICFA.tau[1] - DICFA.tau[0] # TODO replace this by full vector differential and find min() - see README
    ...
    if refine_guess:
        ...
        while(doloop):
            ...
            iq_opt = closestidx(DICFA.q, q_opt)
            q_opt = DICFA.q[iq_opt]
            ...
            kinit = k_opt
            Binit = DICFAtauq[0, iq_opt]
            Ainit = DICFAtauq[-1, iq_opt] - Binit
            p_guess = [Ainit, Binit, kinit]
            p_fit, p_fitcov = curve_fit(ISFmod, DICFA.tau, DICFAtauq[:,iq_opt],
                                        p0 = p_guess)
            k_q = p_fit[2]
            A_q = p_fit[0]
            B_q = p_fit[1]
    ...
    iq_low = closestidx(DICFA.q, q_low)
    q_low = DICFA.q[iq_low]

    q_high = np.sqrt(k_high/D_guess)
    iq_high = closestidx(DICFA.q, q_high)
    q_high = DICFA.q[iq_high]
    ...
    if verbose:
        ...
        print('q_max: ',DICFA.q[-1],'µm-1')

    assert iq_low < iq_opt < iq_high, "unexpected index ordering"

    k_q = np.zeros_like(DICFA.q)
    A_q = np.zeros_like(DICFA.q)
    B_q = np.zeros_like(DICFA.q)
    #
    ...
    for iqf in range(iq_low, iq_opt+1):
        ...
        qf = DICFA.q[iqf]
        kinit = D_guess*qf**2
        Binit = DICFAtauq[0, iqf]
        Ainit = DICFAtauq[-1, iqf] - Binit
        p_guess = [Ainit, Binit, kinit]
        p_fit, p_fitcov = curve_fit(ISFmod, DICFA.tau, DICFAtauq[:,iqf],
                                    p0 = p_guess)
        k_q[iqf] = p_fit[2]
        A_q[iqf] = p_fit[0]
        B_q[iqf] = p_fit[1]
        ...
        if (iqf==iq_low):
            result.ISFmodelfit_qlow = ISFmod(DICFA.tau, *p_fit)

        if (iqf==iq_opt):
            result.ISFmodelfit_qopt = ISFmod(DICFA.tau, *p_fit)
    #
    ...
    for iqf in range(iq_opt+1, iq_high+1):
        ...
        itau_opt = closestidx(DICFA.tau, tau_opt)
        tau_opt = DICFA.tau[itau_opt]
        # fit
        try:
            p_fit, p_fitcov = curve_fit(ISFmod,
                                        DICFA.tau[:itau_opt], DICFAtauq[:itau_opt,iqf],
                                        p0 = p_guess)
        except RuntimeError:
    ...
    p_fit[0] = A_q[iq_high]
    p_fit[1] = B_q[iq_high]
    result.ISFmodelfit_qhigh = ISFmod(DICFA.tau, *p_fit)
...
class DICFanalysis_simple_brownian_result:
    ...
    def show_ISF_radavg(self):
        ...
        for itau in range(1,11):
            plt.plot(self.q[1:],self.radISF_tauq[itau,1:])
        for itau in range(20,len(self.tau),10):
            plt.plot(self.q[1:],self.radISF_tauq[itau,1:])
        plt.xlabel('q [µm-1]')
        plt.ylabel('ISF')
        plt.subplot(212)
        plt.imshow(self.radISF_tauq[1:,1:].T, origin = 'lower', aspect ='auto',
                   extent =(self.tau[0], self.tau[-1],
                            self.q[1], self.q[-1]))
        plt.colorbar()
        plt.ylabel('q [µm-1]')
        plt.xlabel('tau [s]')
    ...
    def show_fits(self):
        ...
        plt.plot(self.tau, self.radISF_tauq[:,self.iq_low],'o')
        plt.plot(self.tau, self.ISFmodelfit_qlow)
        plt.ylabel('ISF')
        plt.subplot(312)
        plt.plot(self.tau, self.radISF_tauq[:,self.iq_opt],'o')
        plt.plot(self.tau, self.ISFmodelfit_qopt)
        plt.ylabel('ISF')
        plt.subplot(313)
        plt.plot(self.tau, self.radISF_tauq[:,self.iq_high],'o')
